 ...
 * Works with OR without a database (falls back to in-memory on Vercel).
 */
export const POST = apiHandler(async () => {
  // If no database available (Vercel), use in-memory fallback
  ...
  // Database available — use real DB
  try {
    // Ensure plans exist
    ...
    if (!user) {
      user = await db.user.create({
        data: { email: 'user@teamgen.app', name: 'User', role: 'USER', credits: 100 },
      })
